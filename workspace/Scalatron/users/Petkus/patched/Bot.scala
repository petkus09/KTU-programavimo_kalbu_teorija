package scalatron.botwar.botPlugin.Petkus
...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, goodsCount, goodForBot, slaveCount) = analyzeViewAsMaster(bot.view)
        ...
        bot.status(goodsCount.toString)
        if (bot.energy > 600  && slaveCount < 6){
            goodForBot match{
                case None =>
                case Some(relPos) => 
                    var unitDelta = relPos.signum
                    var remainder = relPos - unitDelta
                    bot.spawn(unitDelta, "mood" -> "Gathering")
            }
        } 
    }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Gathering" => 
                if (bot.energy >= 3000 || bot.time > 900){
                    bot.set("mood" -> "Returning")
                    reactAsReturningBot(bot)
                }
                else{
                    reactAsGatheringMissile(bot)
                }
            case "Returning" => 
                reactAsReturningBot(bot)
    ...
    def reactAsGatheringMissile(bot: MiniBot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave, goodsCount, goodForBot, slaveCount) = analyzeViewAsSlave(bot.view)
        val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
        if (bot.energy > 300 /*&& goodsCount > 3*/ && slaveCount < 3){
            goodForBot match{
                case None =>
                case Some(relPos) => 
                    var unitDelta = relPos.signum
    ...
    def reactAsReturningBot(bot: MiniBot) {
        val (directionValue) = returnToMaster(bot.view, bot.offsetToMaster)
        val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
        //directionValue(lastDirection) += 10 // try to break ties by favoring the last direction
        val bestDirection45 = directionValue.zipWithIndex.maxBy(_._1)._2
        val direction = XY.fromDirection45(bestDirection45)
        bot.move(direction)
        bot.set("lastDirection" -> bestDirection45)
        //bot.offsetToMaster match {
        //    case Some(delta: XY) =>
                //bot.move(bot.offsetToMaster)
                //bot.set("rx" -> bot.offsetToMaster.x, "ry" -> bot.offsetToMaster.y)
        //}
    }
    ...
    def analyzeViewAsMaster(view: View) = {
        ...
        var slaveCount : Int = 0
        var goodForBot : Option[XY] = None
            ...
            if(cellRelPos.isNonZero) {
                ...
                val value: Double = cells(i) match {
                    ...
                    case 'B' => // good beast: valuable, but runs away
                        goodsCount += 1
                        if (stepDistance > 4 && stepDistance < 9) goodForBot = Some(cellRelPos)
        ...
        (directionValue, nearestEnemyMaster, nearestEnemySlave, goodsCount, goodForBot, slaveCount)
    }
    ...
    def returnToMaster(view: View, master: XY) = {
        val directionValue = Array.ofDim[Double](8)
        ...
        val masterDirection45 = master.toDirection45
        directionValue(masterDirection45) += 500
        (directionValue)
    }
}
